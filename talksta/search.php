	   ...
	   //boundary by organization
        ...
        <tr>
            ...
            <?php  
				...
				?>
                <?php
				$result3=mysqli_query($con,"select accept_status from message_unknown WHERE ((sender_id='$_SESSION[user_id]') AND (receiver_id='$row[id]')) OR ((sender_id='$row[id]') AND (receiver_id='$_SESSION[user_id]')) ");
				$row3=mysqli_fetch_array($result3);
				if($row3['accept_status']=="decline")
				{
				?>
                <td style="color:#F00;"><?php echo "Declined"; ?></td>
                <?php
				}
				else
				{
				?>
                <td><a href="send_unknown.php?receiver_id=<?php echo $row['id']; ?>&&receiver=<?php echo $row['email']; ?>&&receiver_name=<?php echo $row['fname']; ?>&&status1=send">Message</a></td>
